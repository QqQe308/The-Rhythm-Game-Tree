 ...
 passiveGeneration(){
  ...
  doReset(resettingLayer) {
        if (layers[resettingLayer].row > layers[this.layer].row) {
            ...
            if (resettingLayer == "m") {
                if (hasMilestone("m", 1)||hasMilestone("c", 0)) {kept.push("challenges")}
            }
            if (resettingLayer == "c") {
...
upgrades: {
   ...
   return player.p.points.add(10).log(10).pow(1.2)},
    ...
    canAfford() {return player.potential.gte(10.8)},
...
addLayer("l", {
  ...
  softcapPower:new Decimal(0.05),
    ...
    effect() {return player.potential.add(10).log(10).min(1.25)},
    effectDisplay() { return "^"+format(upgradeEffect(this.layer, this.id))},
    cost: new Decimal(3),
    unlocked() {return hasMilestone('l',2)}, },
    13:{ title: "小潜力股",
    ...
    effect() {return player.rks.add(9).log(10).min(1.1)},
    effectDisplay() { return "^"+format(upgradeEffect(this.layer, this.id))},
    cost: new Decimal(3),
    unlocked() {return hasUpgrade('p',37)}, },
   },
...
tabFormat: {
    "里程碑": {
        content: [ ["infobox","introBox"],
          "main-display",
    "prestige-button",
        ["display-text",function() {return '软上限：3诗篇，15诗篇！'},
 ...
 // mult = new Decimal(0)
  ...
  softcapPower:new Decimal(0.15),
    ...
    clickables:{
      ...
      canClick() {return rksRandom()<0.3},
      onClick() {player.rks=player.rks.add(player.rksMax.sub(player.rks).div(5))},
    },
      12: {
      title() {return "增加RKS上限"},
      display() {return "基于Phidata增加RKS上限"},
      canClick() {return rksRandom()>0.7},
      onClick() {player.rksMax = player.rksMax.add(player.p.points.add(1).log(20).add(1).log(10).sub(player.rksMax).div(2).max(0))},
      unlocked() {return hasUpgrade('p',37)}
    },
...
addLayer("c", {
 ...
 introBox: {
    ...
    effectDescription() { return "生成"+format(player.c.points.add(1).pow(2).sub(1))+" Cytus能量每秒"
    },
    gainMult() { 
        mult = new Decimal(1)
        if(player.c.points.gte(1)) mult = new Decimal(0)
        return mult
    },
    gainExp() { 
      exp= new Decimal(0.5)
    ...
    layerShown(){ return true},
    update(diff) {
			if (player.c.unlocked) player.c.power = player.c.power.add(new Decimal(tmp.c.effect).mul(diff));
		},
		upgrades: {
    11:{ title: "我想你懂得",
    description:"解锁RKS（在Phidata界面），Phidata×1e5（软上限前）",
    ...
    0: {
        ...
        effectDescription: "永久自动购买歌曲、源点、诗篇升级，保留Arcaea挑战",
        done() { return player.c.power.gte(1000) }
    },
    1: {
        requirementDescription: "2333 Cytus力量",
        effectDescription: "保留诗篇里程碑和魔王曲里程碑",
        unlocked() {return hasMilestone('c',0)},
        done() { return player.c.power.gte(2333) }
    },
    2: {
        requirementDescription: "2 Cyten && 5000 Cytus力量",
        effectDescription: "解锁？？？（待开发）",
