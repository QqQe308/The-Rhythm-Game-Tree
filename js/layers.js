...
upgrades: {
   ...
   return player.p.points.add(10).log(10).pow(1.2)},
    ...
    37:{ 
      ...
      onPurchase() {confirm("…<br>下一纪元，Cytus。")},
...
addLayer("p", {
    ...
    p1: {
    title: "Phigros•冰封世界",
    body(){ return "本层级所有剧情来自Phigros官方！<br>楔子<br>曾有流浪人，未受洗礼，未信神。<br>攀似绿箩藤，圣石登宵，持剑诰。<br>我要这，穹顶降镐，地霜共冰消，渊无饥骨，路非遥。<br>垂髫闻，沉思莞尔，教后生：何不，振臂筑塔，通云霄。——2089.10.27"},
 ...
 // mult = new Decimal(0)
  ...
  softcapPower:new Decimal(0.15),
    ...
    13:{ title: "别搞时间墙了",
    description:"解锁Phidata挑战和新的剧情，被动获取歌曲和源点速度*5<br>接下来会根据你的游戏进度解锁更多剧情",
    cost: new Decimal(3),
    ...
    0: {
        ...
        effectDescription: "源点挑战要求为0，被动获取歌曲和源点增加500%每秒（PTT永远不重置）<br>解锁Phidata剧情",
        done() { return player.p.points.gte(1) }
    },
    1: {
...
addLayer("c", {
 ...
 introBox: {
    ...
    exponent: 1, 
    effect(){
      return {Notes: player.c.points.add(1).pow(2)}
    },
    effectDescription() { return "Phidata获取量乘以"+format(player.c.points.add(1).pow(2))
    },
    gainMult() { 
        mult = new Decimal(1)
        return mult
